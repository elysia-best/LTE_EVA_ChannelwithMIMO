...
msg_pilot = transpose(reshape(msg_qpsk, pilotfrequency, length(msg_qpsk)/pilotfrequency));  % trick, 添加导频方便
...
msg_casd_len = numel(msg_casd);
original_size = (length(msg_qpsk)/pilotfrequency) * (pilotfrequency+1);
msg_pilot_original = msg_casd(1 : original_size); % Corresponding to msg_pilot_aft_casd
